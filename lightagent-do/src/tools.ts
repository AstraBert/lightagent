...
export class GrepTool extends ToolFunction<FileSystem> {
  ...
  readonly inputSchema = v.object({
    ...
    pattern: v.pipe(
      v.string(),
      v.description("Regex pattern to grep the file for"),
    ),
    matches: v.pipe(
      v.optional(v.number()),
      v.description(
        "Number of matches to return. Defaults to returning all found matches.",
      ),
    ),
    context: v.pipe(
      v.optional(v.number()),
      v.description(
        "Number of characthers to preserve before and after the match, for context. Defaults to 0.",
      ),
    ),
  });
    ...
    try {
      ...
      if (matches.length) {
        return {
          type: "success",
          result:
            `No match was found for \`${validated.pattern}\` in ${validated.path}`,
        };
      }
      ...
      return {
        type: "error",
        error: `An error occurred while running the \`grep\` tool: ${e}`,
      };
    }
  }
}
...
export class ReadDirTool extends ToolFunction<FileSystem> {
  ...
  readonly description: string =
    "Read a directory and get its children (files and sub-directories)";
  readonly inputSchema = v.object({
    directory: v.pipe(
      v.string(),
      v.description("Path of the directory to read"),
    ),
    recursive: v.pipe(
      v.optional(v.boolean()),
      v.description(
        "Whether or not to recursively read the directory. Defaults to false.",
      ),
    ),
  });
    ...
    try {
      ...
      const readDir = async (
        ...
        for await (const entry of this.ctx.readDir(directory)) {
          if (entry.isFile) {
            entries.push(
              `Path: ${path.join(directory, entry.name)} - Type: file`,
            );
            ...
            if (recursive) {
              ...
              entries.push(
                `Path: ${path.join(directory, entry.name)} - Type: directory`,
              );
            }
          }
        }
      ...
      return {
        type: "error",
        error: `An error occurred while running the \`read_dir\` tool: ${e}`,
      };
    }
  }
}
...
export class UnixTool extends ToolFunction<FileSystem> {
  ...
  readonly description: string =
    "Tool to execute a small group of readonly unix tools: pwd, cat, head, date, diff.";
  readonly inputSchema = v.object({
    ...
    head: v.pipe(
      v.optional(v.object({
        n: v.pipe(
          v.number(),
          v.description("Number of lines to read from the head of the file"),
        ),
  ...
  async execute(input: JsonValue): Promise<ToolResult> {
    try {
      ...
      if (typeof validated.cat !== "undefined") {
        ...
        const lines = await this.ctx.readLines(
          validated.head!.file,
          validated.head!.n,
        );
      ...
      return {
        type: "error",
        error: `An error occurred while running the \`unix_tools\` tool: ${e}`,
      };
    }
  }
}
