...
import { textMessage, Message, MessageRole, MessagePart } from "@cle-does-things/llms-sdk";
...
const ToolResultSchema = v.union([v.object({type: v.literal("success"), result: v.string()}), v.object({type: v.literal("error"), error: v.string()})])
...
const AssistantMessagePartSchema = v.union([TextPartSchema, ThinkingPartSchema, ToolCallPartSchema]);
...
export const AgentEventSchema = v.union([SessionInitEventSchema, SessionStopEventSchema, UserPromptSubmitEventSchema, SkillLoadEventSchema, ToolCallAnyEventSchema, ToolCallEventSchema, AssistantResponseEventSchema, ToolResultEventSchema, StreamDeltaEventSchema])
...
export function convertEventsToMessages(events: AgentEvent[]): Message[]  {
  ...
  for (const event of events) {
    switch (event.type) {
        ...
        for (const c of event.content) {
          switch (c.type) {
            case "text":
              ...
              break
            case "thinking":
              parts.push({ thinking: c.thinking, signature: c.signature, type: "thinking" })
              break
            case "toolCall":
              parts.push({ type: "toolCall", id: c.id, arguments: c.arguments, name: c.name })
              break
          }
        }
      ...
      case "tool.call_any":
        messages.push({ role: "assistant" as MessageRole, content: [{type: "toolCall", id: event.toolCallId, name: event.name, arguments: JSON.stringify(event.input) }]})
        break
      case "tool.result":
        messages.push({ role: "tool" as MessageRole, content: [{type: "toolResult", toolCallId: event.toolCallId, result: event.result.type === 'success' ? event.result.result : `Tool call failed\n${event.result.error}`}]})
        break
      default:
        break
    }
