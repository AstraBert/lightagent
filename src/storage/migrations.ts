...
function getCurrentMigration(db: Database): number {
  const stmt = db.prepare("select name from sqlite_master where type='table' and name='_migrations'")
  const exists = stmt.get();
  if (typeof exists === "undefined") {
    return 0;
  }
  const versionStmt = db.prepare("select version, applied_at from _migrations order by applied_at desc, version desc");
...
export async function applyMigrations(db: Database): Promise<void> {
  ...
  for (const m of toApply) {
    ...
    db.exec("insert into _migrations (version, applied_at) values (:version, :timestamp)", { version: m.version, timestamp: Number(timestamp) })
  }
}
